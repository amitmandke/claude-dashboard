...
15. **Switch theme** — 🌙/☀️ in the header toggles between the dark (default) and light themes; the choice persists across visits.
...
- **Dark and light themes via CSS variables only** — every color in `style.css` lives in a variable on `:root` (dark, the default) with a complete counterpart under `[data-theme="light"]`; no rule hardcodes a color. The 🌙/☀️ header toggle flips `data-theme` on `<html>`, persisted in `localStorage` and applied by an inline `<head>` script before the stylesheet loads (no flash). The reply popup follows the "Markdown Reader" extension's matching theme pair (one-dark / one-light). Deliberate exception: the terminal mirror stays dark in both themes — it mirrors a real terminal pane.
